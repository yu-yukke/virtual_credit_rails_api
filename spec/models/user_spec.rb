...
RSpec.describe User do
  ...
  describe 'published' do
    context 'when user is not published' do
      ...
      it 'can not find user' do
        expect(described_class.published.find_by(id: user.id)).to be_nil
      end
    end
    ...
    context 'when user is published' do
      ...
      it 'can find user' do
        expect(described_class.published.find_by(id: user.id)).to eq(user)
      end
    end
  end
  ...
  describe '.cover_image_url' do
    context 'when user has cover_image' do
      let_it_be(:user) { create(:user, :activated) }

      it 'returns cover_image_url' do
        expect(user.cover_image_url).to be_a(String)
      end
    end

    context 'when user does not have cover_image' do
      let_it_be(:user) { create(:user, :confirmed) }

      it 'returns nil' do
        expect(user.cover_image_url).to be_nil
      end
    end
  end
  ...
  describe '.copyrighted_works' do
    context 'when user has no copyrighted works' do
      let_it_be(:user) { create(:user, :activated) }

      it 'returns empty array' do
        expect(user.copyrighted_works).to be_empty
      end
    end

    context 'when user has copyrighted works' do
      let_it_be(:user) { create(:user, :activated, :with_copyrights) }

      it 'returns copyrighted works' do
        expect(user.copyrighted_works).not_to be_empty
      end
    end
  end
  ...
  describe '.related_works' do
    context 'when user has no works and copyrighted works' do
      let_it_be(:user) { create(:user, :activated) }

      it 'returns empty array' do
        expect(user.related_works).to be_empty
      end
    end

    context 'when user has works and copyrighted works' do
      let_it_be(:user) { create(:user, :activated, :with_works, :with_copyrights) }

      it 'returns related works' do
        expect(user.related_works).not_to be_empty
      end
    end
  end
end
